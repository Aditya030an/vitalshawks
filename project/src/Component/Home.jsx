...
import { Clock, Users, BarChart, Target, Upload, Database, Headphones, Rocket } from "lucide-react";
const services = [
    {
    ...
    <section className="relative w-full min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0e2338] via-[#1c3c59] to-[#27496d] text-white px-6">
      <div className="max-w-4xl text-center">
        ...
        <h1 className="text-4xl md:text-6xl font-extrabold leading-tight tracking-tight mb-6">
          Build Your Sales Pipeline with <span className="text-orange-400">Verified B2B Databases</span>
        </h1>

        {/* Subheadline */}
        <p className="text-lg md:text-xl text-gray-200 mb-10 leading-relaxed">
          We provide accurate contact information for your ideal customers and handle the backend work — 
          so your sales team can focus on outreach and closing deals effectively.
        </p>

        {/* Call To Action */}
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button className="bg-orange-500 hover:bg-orange-600 transition px-8 py-3 rounded-lg font-semibold shadow-md">
            Get Started
          </button>
          <button className="bg-transparent border border-white hover:bg-white hover:text-[#0e2338] transition px-8 py-3 rounded-lg font-semibold">
            Learn More
          </button>
        </div>
      </div>
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10">
      {services.map((service, index) => (
        <motion.div
          ...
          viewport={{ once: true }}
          className="group bg-gray-50 p-8 rounded-3xl shadow-lg hover:scale-110 hover:shadow-2xl transition-all duration-500"
        >
...
<div className="bg-white text-gray-800">
      ...
      <section className="relative max-w-6xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Card with soft background & border */}
          {[
            { icon: <Clock />, title: "Time-Saving", desc: "Stop spending hours finding the right contacts." },
            { icon: <Users />, title: "Smart Support", desc: "Our experienced team handles backend research so your sales team can focus on selling." },
            { icon: <BarChart />, title: "Higher Effectiveness", desc: "Too many tasks reduce results — we streamline your operations." },
            { icon: <Target />, title: "Custom Targeting", desc: "Reach the right companies and decision-makers that matter most." },
          ].map((item, i) => (
            <motion.div
              key={i}
      ...
      <section className="relative bg-gradient-to-r from-[#0e2338] via-[#16314d] to-[#0e2338] text-white py-20 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 text-center">
            {[
              { icon: <Upload />, title: "Share Your Target Audience", desc: "Tell us who your ideal customers are." },
              { icon: <Database />, title: "We Build Your Database", desc: "Fresh, accurate, decision-maker contacts." },
              { icon: <Headphones />, title: "Backend Support", desc: "We handle research so your team can focus on outreach." },
              { icon: <Rocket />, title: "Boost Sales Effectiveness", desc: "Less busywork, more conversions." },
            ].map((step, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 50 }}
          ...
          <div className="text-center mt-16">
            <button className="px-10 py-4 bg-[#B77F59] text-[#0e2338] font-semibold rounded-2xl shadow-lg hover:bg-yellow-300 transition">
